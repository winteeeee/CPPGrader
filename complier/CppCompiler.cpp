...
void CppCompiler::실행기_작동(const std::string &디렉토리_경로,
                              const std::string &임시_파일명,
                              const std::string &명령어,
                              const TestCase &테스트케이스들,
                              const std::vector<std::string> &목표_소스코드) const {
    const std::vector<std::string> 입력파일들 = 테스트케이스들.get입력파일들();
    const std::vector<std::string> 정답파일들 = 테스트케이스들.get정답파일들();
    const std::vector<std::string> 명령행_인수들 = 테스트케이스들.get명령행인수들();
...
CppCompiler::CppCompiler(std::ofstream& 출력_스트림)
: 출력_스트림(출력_스트림), 실행기(출력_스트림) {}

void CppCompiler::컴파일(const std::string &디렉토리_경로,
                         const std::string &학생_정보,
                         const std::vector<std::string> &소스코드들,
                         const std::vector<TestCase> &테스트케이스들,
                         const int &옵션,
                         const int &인덱스) const {
    std::string 임시_파일명 = "output.txt";
    std::string 명령어;
    ...
    if (옵션 == CppCompiler::컴파일_옵션::일반) {
        ...
        for (const std::string &소스코드: 소스코드들) {
            if (소스코드.find(".cpp") != std::string::npos) {
                명령어 += (소스코드 + " ");
            }
        }
        명령어 += " -o output";
        ...
        for (const auto &소스코드 : 소스코드들) {
            std::vector<std::string> 임시_벡터 = {소스코드};
            if (소스코드.find(".cpp") != std::string::npos) {
                명령어 = "g++ " + 소스코드 + " -o output";
            }
